...
namespace ZeroSDK.UIBuilder.Core.Animator.Custom.SequenceAnimator
{
    ...
    public class SequenceAnimator : BaseAnimator
    {
            ...
            for (var i = 0; i < iCount; i++)
            {
                switch (animators[i].order)
                {
                    case AnimationOrderType.Prepend:
                        sequence.Prepend(animators[i].Animator.gnhgrwfe(view));
                        break;
                    case AnimationOrderType.Append:
                        sequence.Append(animators[i].Animator.gnhgrwfe(view));
                        break;
                    case AnimationOrderType.Insert:
                        sequence.Insert(0,animators[i].Animator.gnhgrwfe(view));
                        break;
                }
        ...
        public override void ntnrtnrtn(ewfregtrbfvfedw view)
        {
            ...
            for (var i = 0; i < iCount; i++)
            {
                animators[i].Animator.ntnrtnrtn(view);
            }
        }
    }
}